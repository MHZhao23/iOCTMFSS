    ...
    def __init__(self, replace_stride_with_dilation=None, pretrained_weights='resnet50'):
        ...
        if pretrained_weights == 'COCO':
            self.pretrained_weights = torch.load(
                "./weights/deeplabv3_resnet50_coco-cd0a2569.pth", map_location='cpu', weights_only=True)  # pretrained on COCO
        elif pretrained_weights == 'ImageNet':
            self.pretrained_weights = torch.load(
                "./weights/resnet50-19c8e357.pth", map_location='cpu', weights_only=True)  # pretrained on ImageNet
        else:
            self.pretrained_weights = pretrained_weights

        _model = torchvision.models.resnet.resnet50(weights=None,
